(function() {
  ...
  var jsq = root.jsq = _.extend(
    function(scope) { this.$s = scope.$s; return this; },
    {
      out:null,
      err:null,
      loader:null,
  ...
  Cls.prototype.includes = function(def){
      ...
      _.extend(fun, def);
      var hierachy = classHierarchy(_(fun).clone());
      _(hierachy).each(function(ancestor) {
        _.extend(fun, ancestor);
      });
      return this;
  };
  ...
  Cls.prototype.defines = function(def) {
      ...
      jsq.out.println("==========================");
      jsq.out.println(name);
      jsq.out.println(_(hierachy).map(function(elem) {
          return elem.$s ? elem.$s._name : undefined;
      }, this));
      jsq.out.println("==========================");

      hierachy = _(hierachy).map(function(elem) {
  ...
  function classHierarchy(factory) {
      if(!factory) return [];
      var parent = factory.$s? factory.$s._parent : undefined,
          hierarchy = classHierarchy(parent);
      hierarchy.push(factory);
      return hierarchy;
  };
  ...
  function baseWrapper(proto) {
      ...
      _(proto).chain().keys().each(function(key) {
        ...
        wrapper[key] = function() {
          func.apply(this.invoker, arguments);
        }
      }, proto);
      return wrapper;
  };
